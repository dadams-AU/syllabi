...
% Ensure you have the required packages installed in your LaTeX distribution
% !TEX program = lualatex
% Document metadata for accessibility and compliance
\DocumentMetadata{
  pdfstandard=UA-2,   % Use UA-1 + pdfversion=1.7 if your validator requires it
  pdfversion=2.0,     % PDF version 2.0 for modern accessibility features
  lang=en-US          % Language set to US English
}

% Document class declaration
\documentclass[12pt]{article} % Use 12pt font size and article class
...
% ---- Option A (default): All-sans body; bolder, spaced headings
\newcommand{\useSansHeadings}{%
  % Section numbers already set earlier; this just styles faces/sizes/spacings.
  \titleformat{\section}
    {\sffamily\bfseries\Large\addfontfeatures{LetterSpace=2.0}}{\thesection}{0.6em}{}
  \titleformat{\subsection}
    {\sffamily\bfseries\large}{\thesubsection}{0.6em}{}
...
% ---- Option B: Serif body text; keep headings in sans for contrast
\newcommand{\useSansHeadingsWithSerifBody}{%
  \titleformat{\section}
    {\sffamily\bfseries\Large\addfontfeatures{LetterSpace=2.0}}{\thesection}{0.6em}{}
  \titleformat{\subsection}
    {\sffamily\bfseries\large}{\thesubsection}{0.6em}{}
  ...
  \titlespacing*{\subsubsection}{0pt}{0.9ex plus .3ex}{0.4ex}
}

% === Activate one of the two ===
\useSansHeadings
% \useSansHeadingsWithSerifBody  % <- use this if you switch to a serif main font
...
% Global, accessible list defaults
\setlist[itemize]{itemsep=0pt, parsep=0pt, topsep=0.25\baselineskip}
% Bulletless, flush-left list for metadata lines
\newlist{flatlist}{itemize}{1} % Define a new list type
\setlist[flatlist]{label={}, leftmargin=0pt, itemsep=0pt, parsep=0pt, topsep=0pt}

% Figures, tables, and captions
\usepackage{graphicx} % Enable inclusion of graphics
\usepackage{array, booktabs, longtable} % Table formatting packages
\usepackage{caption} % Customize captions for figures and tables
\usepackage{tabularray} % Enhanced table formatting
\UseTblrLibrary{booktabs} % Enable booktabs-style rules for tables
...
% Color definitions
\usepackage{xcolor} % Enable color customization
% High-contrast annotation colors (WCAG AA compliant on white background)
\definecolor{annotationblue}{RGB}{0,77,153} % Define annotation blue color (#004D99)
\definecolor{suggestionred}{RGB}{153,0,0}   % Define suggestion red color (#990000)

% Hyperlinks and PDF metadata
\usepackage[
  pdfusetitle,           % Use title from \title command
  pdflang=en-US,         % Set PDF language to US English
  pdfstartview=FitH,     % Start PDF with full width view
...
\hypersetup{
  ...
  pdftitle={PREFIX \& Number: Title}, % Set PDF title
  pdfauthor={Instructor Name},        % Set PDF author
  pdfsubject={California State University, Fullerton Course Syllabus}, % Set PDF subject
  pdfkeywords={CSUF, Syllabus, Accessibility}, % Set PDF keywords
  pdfborderstyle={/S/U/W 1} % Set link border style to underline
}
\urlstyle{same} % Use the same font style for URLs

% Document information
\title{PREFIX \& Number: Title} % Placeholder for course title
\author{} % Leave author blank
\date{Semester, Year} % Placeholder for semester and year

% Accessibility enhancements
\setlistdepth{3} % Set maximum list depth for accessibility
% Example alt text for images: \includegraphics[width=2.75in, alt={Cal State Fullerton wordmark}]{csuf_logo.png}
...
\noindent {\color{suggestionred}See Table \ref{tab:grading_scale} below for one example:}
...
\begin{table}[h]
  ...
  \begin{tblr}{
    colspec = {l c},
    rowhead = 1,                 % first row = <TH> scope="column"
    row{1} = {font=\bfseries},
  }
...
\begin{table}[h]
  ...
  \begin{tblr}{
    colspec = {l c},
    rowhead = 1,                 % first row is header
    row{1} = {font=\bfseries},   % bold header
  }
  Item & Points = \% \\
  Participation & 25 \\
  Presentation & 25 \\
...
% Section 8A: AI Policies (required)
\section*{AI Policies} {\color{annotationblue}For suggestions, please refer to \href{https://fdc.fullerton.edu/teaching/ai/creating%20policy.html}{FDC AI Policy Resources}}
...
\noindent {\color{annotationblue}Alternatively, you may format the calendar as a table, as shown in Table \ref{tab:schedule_of_classes}. Follow the instructions on the guide ``Creating an Accessible Table'' (linked from the Course Development Resources tab).}
