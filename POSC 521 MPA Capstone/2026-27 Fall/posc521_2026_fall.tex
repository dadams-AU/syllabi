...
\noindent \textbf{Every assignment below carries an AI label.} \textbf{RED} means no AI. \textbf{YELLOW} means AI is permitted at one specified point inside the assignment, under an approved prompt, with the transcript in your Feedback Appendix. Nothing in this course is Green, and only two things are Yellow. Table \ref{tab:ai-labels} gives the full definitions and Table \ref{tab:ai-where} puts every assignment in one place.

\subsection*{1) Weekly Readings Assignment}

\textbf{Important Note:} Beginning in Week 2, every student includes a \emph{Feedback Appendix} with the annotated bibliography every week one is assigned, and with the final synthesis paper in every week that has one. No optional weeks and no exceptions on either. Nothing else in the course carries an appendix, because nothing else in the course permits AI at all; Table \ref{tab:ai-where} lists every graded assignment and where the line falls.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Answer three questions about your own paper, put to you cold.} Why these sources, why this position on the tradeoff, and what the position costs and who pays it. You may consult your draft but not read from it. The standard stated throughout this syllabus---that work you cannot defend does not count as yours---applies here to everyone equally rather than to whoever happens to draw suspicion.
    \end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Defense of your own work} (12/7 only). You answer cold questions about your concentration paper out of the paper's own logic rather than out of a summary you memorized.
\end{enumerate}
...
\paragraph*{No AI, and the reason is structural rather than a rule.} Nothing in this component is submitted, so there is nothing to run through a tool. That is the point of putting weight here. Table \ref{tab:ai-where} lists it as such for completeness.
